...
function _via_util_get_button(icon_id, title, id, width) {
    ...
    el.onmouseleave = function leave_msg() {
        document.getElementById('_via_message_container').style.display = 'none';
    };
    return el;
}

function _via_util_get_input(icon_id, title, id) {
    var el = document.createElement('INPUT');
    el.id = icon_id;
    el.style = "width:24px; height:24px";
    el.value = '1';
    el.onmouseover = function over_msg() {
        _via_util_msg_show(title);
    };
...
function _via_util_get_label(icon_id, title, id) {
...
function _via_util_post_file_to_server(data, timeStamp, mode = 'auto') {
    // simulate a mouse click event
    let formData = new FormData();
    formData.append('anno', data);
    let log_string = {};
    log_string['time'] = timeStamp;

    let ts = new Blob([JSON.stringify(log_string)], {type: 'text/json;charset=utf-8'});
    formData.append('log', ts);
    ...
    $.ajax({
        ...
        success: function (res) {
            if (mode == 'auto') {
                _via_util_msg_show('Automatic annotation saving is successful !');
            } else {
                _via_util_msg_show("Save annotation: " + res['m']);
            }
            // 显示success的提示框
        },
        error: function (res) {
            _via_util_msg_show('Save annotation by ' + mode + ": "+ res['m']);
        }
    });
}
