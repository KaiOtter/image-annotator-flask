    ...
    return new Promise(function (ok_callback, err_callback) {
        try {
            ...
            for (var i = 0; i < filelist.length; ++i) {
                ...
                // vid is equal to fid when no deletion happens (I guess...)
...
// Customize function
_via_data.prototype.project_save_server = function (mode) {
    return new Promise(function (ok_callback, err_callback) {
        try {
            // @todo: decide on whether we want to include the base64 data
            // of inline files (i.e. this.store.file[fid].loc === _VIA_FILE_LOC.INLINE)
            ...
            //   filename.push(this.store.project.pid.substr(0,8) + '_');
            // }
            let time_stamp = _via_util_date_to_filename_str(Date.now());
            _via_util_post_file_to_server(data_blob, time_stamp, mode);
            ok_callback();
        } catch (err) {
            _via_util_msg_show('Failed to save project! [' + err + ']');
            err_callback();
