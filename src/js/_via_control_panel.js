...
function _via_control_panel(control_panel_container, via) {
    ...
    this.via.vm._on_add_media_server();
    this._init();
    this.saveTimer = undefined;
    this.init_auto_save();
}

_via_control_panel.prototype.init_auto_save = function () {
    var el = document.getElementById('autosave_input');
    var bar_el = document.getElementById('bar_dynamic');
    let minute = parseInt(el.value) * 1000 * 60;
    var width = 0;
    clearInterval(this.saveTimer);
    this.saveTimer = setInterval(show_bar, minute / 40)

    function show_bar() {
        if (width == 40) {
            clearInterval(this.saveTimer);
            this.via.d.project_save_server('auto');
            width = 0;
            bar_el.style.width = '0px'
        } else {
            width = width + 1;
            bar_el.style.width = width + 'px'
        }
    }
...
_via_control_panel.prototype._init = function (type) {
    ...
    var back_home = document.createElement('DIV');
    back_home.setAttribute('class', 'logo');
    back_home.innerHTML = '<a href="/" title="go back to home page">HOME</a>';
    this.c.appendChild(back_home)
...
_via_control_panel.prototype._add_view_manager_tools = function () {
    ...
    // var add_media_local = _via_util_get_svg_button('micon_add_circle', 'Load images and annotations from server', 'add_media_local');
...
_via_control_panel.prototype._add_project_tools = function () {
    ...
    var bar_dynamic = document.createElement('DIV');
    bar_dynamic.style.width = '0px';
    bar_dynamic.style.marginLeft = '0px';
    bar_dynamic.style.height = '24px';
    bar_dynamic.id = 'bar_dynamic';
    bar_dynamic.style.backgroundColor = '#4CAF50';
    bar_dynamic.style.position = 'absolute';

    var process_bar = document.createElement('DIV');
    process_bar.id = 'process_bar';
    process_bar.style.width = '40px';
    process_bar.style.height = '24px';
    process_bar.style.position = 'relative';
    process_bar.style.backgroundColor = '#ddd';
    process_bar.appendChild(bar_dynamic)

    this.c.appendChild(process_bar)
...
// </style>
    ...
    var set_auto_save = _via_util_get_input('autosave_input', 'interval minutes of auto-save', 'set_auto_save');
    set_auto_save.addEventListener('focusout', function () {
        this.init_auto_save();
    }.bind(this));
    this.c.appendChild(set_auto_save);

    var min_tag = _via_util_get_label('min_tag', 'min', 'min_tag');
    this.c.appendChild(min_tag);

    var save = _via_util_get_svg_button('micon_save', 'Save annotation to Server');
    save.addEventListener('click', function () {
        // this.via.d.project_save();
        this.via.d.project_save_server('active');
    }.bind(this));
    this.c.appendChild(save);

    var import_export_annotation = _via_util_get_svg_button('micon_import_export', 'Import or Export Annotations to local');
