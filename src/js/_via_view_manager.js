...
_via_view_manager.prototype._on_next_view = function () {
    if (this.view_selector.options.length) {
        ...
        if (vindex !== -1) {
            ...
            let next_vid = this.view_selector_vid_list[next_vindex];
            var fid = (this.d.store.view[next_vid].fid_list);
            var arg = window.location.search;
            var img_blob;
...
_via_view_manager.prototype._on_prev_view = function () {
    if (this.view_selector.options.length) {
        ...
        if (vindex !== -1) {
            ...
            let prev_vid = this.view_selector_vid_list[prev_vindex];
            var fid = (this.d.store.view[prev_vid].fid_list);
            var arg = window.location.search;
            var img_blob;
...
_via_view_manager.prototype._on_add_media_server = function () {
    // 替代本地文件框的读取，用ajax远程get数据，封装到filelist里面
    ...
    // fsel.accept = 'image/*';
    // fsel.onchange = this._file_add_server;
    // fsel.click();
    ...
    $.ajax({
        ...
        success: function (result) {
            let blob = base64toBlob(result.img[0].image, 'image/JPEG');
            filelist.push({
                'fname': result.img[0].filename,
                'type': _VIA_FILE_TYPE.IMAGE,
                'loc': _VIA_FILE_LOC.LOCAL,
                'src': blob,
            });

            for (var findex = 1; findex < result.img.length; ++findex) {
                // let blob = base64toBlob(result.img[findex].image, 'image/JPEG');
                filelist.push({
                    'fname': result.img[findex].filename,
                    'type': _VIA_FILE_TYPE.IMAGE,
                    'loc': _VIA_FILE_LOC.LOCAL,
                    'src': undefined,
                });
    ...
    if ((anno_archive != null) && (anno_archive != undefined)) {
        //  load annotation from server archive
        this.d.project_load_json(anno_archive);
        // this.d.store = anno_archive;
    } else {
        _via_util_msg_show('No annotation archives on the server !');
    }
}
